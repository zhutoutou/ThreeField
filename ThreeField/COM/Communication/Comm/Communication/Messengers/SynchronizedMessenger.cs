...
namespace ZIT.Communication.Comm.Communication.Messengers
{
    /// <summary>
    ...
    public class SynchronizedMessenger<T> : RequestReplyMessenger<T> where T : IMessenger
    {
        ...
        /// </summary>
        private readonly ManualResetEvent _receiveWaiter;   //ManualResetEventSlim
        ...
        public SynchronizedMessenger(T messenger, int incomingMessageQueueCapacity)
            : base(messenger)
        {
            _receiveWaiter = new ManualResetEvent(false);  //ManualResetEventSlim
        ...
        public IScsMessage ReceiveMessage(int timeout)
        {
            while (_running)
            {
                ...
                var signalled = _receiveWaiter.WaitOne(timeout);
